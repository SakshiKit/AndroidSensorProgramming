...
public class StepsDBHelper extends SQLiteOpenHelper {
    ...
    public boolean createStepsEntry() {
        ...
        String todayDate = String.valueOf(mCalendar.get(Calendar.MONTH) + 1) + "/" + String.valueOf(mCalendar.get
                (Calendar.DAY_OF_MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.YEAR));
        ...
        try {
            ...
            if (c.moveToFirst()) {
                do {
                    ...
                    dateStepsModel.mDate = c.getString((c.getColumnIndex(CREATION_DATE)));
                    dateStepsModel.mStepCount = c.getInt((c.getColumnIndex(STEPS_COUNT)));
